...
export default function Giveaways() {
      ...
      for (const giveaway of channelGiveaways) {
        ...
        if (participants && participants.length > 0) {
          ...
          for (const p of participants) {
            ...
            // Use the new verification function that bypasses RLS
            const { data: isVerified } = await supabase
              .rpc('check_user_verification_by_kick_username', { 
                kick_username_param: p.kick_username 
              });
            
            console.log(`✅ ${p.kick_username} live chat verification: ${isVerified || false}`);
  ...
  const viewUserProfile = async (username: string) => {
    try {
      ...
      // Use the new verification function that bypasses RLS
      console.log(`🔧 Debug: Checking verification for "${username}"`);
      
      const { data: isVerified, error: verificationError } = await supabase
        .rpc('check_user_verification_by_kick_username', { 
          kick_username_param: username 
        });
      
      console.log(`🔧 Debug: Verification result for ${username}:`, { isVerified, verificationError });
      
      console.log(`✅ ${username} verification status: ${isVerified || false} (User verified: ${isVerified || false})`);
  ...
  const refreshVerificationStatus = async (giveawayId: string) => {
    try {
      ...
      for (const participant of participants) {
        // Use the new verification function that bypasses RLS
        console.log(`🔧 Debug: Checking verification for ${participant.kick_username}`);
        const { data: isVerified, error: verificationError } = await supabase
          .rpc('check_user_verification_by_kick_username', { 
            kick_username_param: participant.kick_username 
          });

        console.log(`🔧 Debug: Verification result for ${participant.kick_username}:`, { isVerified, verificationError });
        
        console.log(`🔍 ${participant.kick_username}: verified=${isVerified || false}`);
        
        // Update the participant's verification status
        const { error: updateError } = await supabase
          .from('giveaway_participants')
          .update({ is_verified: isVerified || false })
          .eq('id', participant.id);

        if (updateError) {
          console.error(`❌ Error updating verification for ${participant.kick_username}:`, updateError);
        } else {
          console.log(`✅ Updated verification status for ${participant.kick_username}: ${isVerified || false}`);
        }
      }
