-- Drop the existing function and recreate with correct parameter names that match frontend calls
DROP FUNCTION IF EXISTS public.link_kick_account_to_profile(uuid, text, text, text);

-- Recreate the function with parameter names that match the frontend call
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE profiles.linked_kick_user_id = kick_user_id 
    ...
    SET 
      ...
      kick_username = null,
      updated_at = now()
    WHERE profiles.kick_user_id = kick_user_id 
    AND profiles.user_id != profile_user_id;
  END IF;
