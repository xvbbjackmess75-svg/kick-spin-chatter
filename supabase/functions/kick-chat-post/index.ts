...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.log('🔍 Admin query error:', adminRoleError)

      if (adminRoleError) {
        throw new Error(`Admin query failed: ${adminRoleError.message}`)
      }

      if (!adminRoles || adminRoles.length === 0) {
        console.error('❌ No admin with linked Kick account found')
        return new Response(JSON.stringify({ 
      ...
      console.error('🚨 Error in admin query:', queryError)
      throw queryError
    }
